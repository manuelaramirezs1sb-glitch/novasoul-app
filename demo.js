...
(function () {
  ...
  const ESTADO_DEMO = {
    ...
    'novedad': 'novedad', 'pendiente': 'pendiente',
    'guia generada': 'confirmado', 'en bodega origen': 'en_bodega',
  ...
  async function importarDemo(p) {
    ...
    if (esZip) {
      ...
      matriz = XLSX.utils.sheet_to_json(hoja, { header: 1, raw: false, defval: '' });
    } else {
    ...
    for (let i = 1; i < matriz.length; i++) {
      ...
      if (esPauta) {
        HOJA.Pauta.push({
          ...
          entrega: '', presupuesto: '',
          gasto: num(String(f[col.gasto]).replace(/[^\d.,-]/g, '').replace(',', '.')),
          ...
          ctr: '', cpc: '', cpm: '',
          resultados: col.resultados !== undefined ? num(f[col.resultados]) : 0,
      ...
      HOJA.Pedidos.push({
        ...
        cantidad: col.cantidad !== undefined ? num(f[col.cantidad]) || 1 : 1,
        valor: col.valor !== undefined ? num(String(f[col.valor]).replace(/[^\d.,-]/g, '').replace(',', '.')) : 0,
        costo_producto: col.costo_producto !== undefined ? num(f[col.costo_producto]) : 0,
        costo_envio: col.costo_envio !== undefined ? num(f[col.costo_envio]) : 0,
        metodo_pago: 'contraentrega', bodega: '',
    ...
    return {
      ...
      resumen: nuevas + ' filas entraron al demo. Se ven reflejadas al instante ' +
               'y desaparecen cuando recargues.',
      demo: true,
    };
  }
  ...
  function cinta() {
    ...
    c.innerHTML = 'MODO DEMOSTRACIÓN · datos inventados, nada se guarda' +
      '<button id="demo-cambiar">Cambiar de rol</button>' +
      '<button id="demo-reiniciar">Empezar de nuevo</button>';
    document.body.appendChild(c);
    document.body.classList.add('demo-on');
    document.getElementById('demo-cambiar').onclick = function () { location.reload(); };
